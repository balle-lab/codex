...
def test_perturb_and_apply_returns_function_output():
    ...
    u = jax.random.uniform(rng3, x.shape, minval=-0.5, maxval=0.5)
    chex.assert_trees_all_close(gradient.perturb_and_apply(f, x, u, y), f(x + u, y))
...
def test_perturb_and_apply_does_not_fail_in_flax_modules():
    class InnerWithCall(nn.Module):
        ...
        @override
        def __call__(self, x):
            bias = self.param("bias", nn.initializers.constant(0.25), x.shape)
            return x**2 + bias**3
    ...
    class OuterWithLambda(nn.Module):
        ...
        def __call__(self, x, u):
            inner = InnerWithCall()
            return gradient.perturb_and_apply(lambda x: 1.0 * inner(x), x, u)
    ...
    chex.assert_trees_all_equal(p_dot_call, 0.1875)
    chex.assert_trees_all_equal(p_dot_method, 0.1875)
    chex.assert_trees_all_equal(p_dot_lambda, 0.1875)
