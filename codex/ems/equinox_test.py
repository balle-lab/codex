...
class TestDistributionEntropyModel:
    ...
    def test_shapes_are_correct(self):
        ...
        param_shapes = jax.tree.map(
            lambda p: p.shape, jax.tree.flatten(eqx.filter(model, eqx.is_array))[0]
        )
        chex.assert_trees_all_equal(param_shapes, self.expected_param_shapes)
        ...
        grad_shapes = jax.tree.map(
            lambda g: g.shape, jax.tree.flatten(eqx.filter(grads, eqx.is_array))[0]
        )
        chex.assert_trees_all_equal(grad_shapes, self.expected_param_shapes)
...
class TestPeriodicFourierEntropyModel(TestDistributionEntropyModel):
    ...
    @override
    def get_em(self, rng):
        return ems.PeriodicFourierEntropyModel(rng, period=2.0, num_pdfs=2, num_freqs=5)
