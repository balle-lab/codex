...
class EntropyModelTests:
    ...
    def test_shapes_are_correct(self):
        ...
        chex.assert_trees_all_equal(
            jax.tree.map(lambda p: p.shape, init_params), self.expected_param_shapes
        )
        ...
        chex.assert_trees_all_equal(
            jax.tree.map(lambda g: g.shape, grads), self.expected_param_shapes
        )
...
class TestPeriodicFourierEntropyModel(EntropyModelTests):
    ...
    @override
    def get_em(self):
        return ems.PeriodicFourierEntropyModel(period=2.0, num_pdfs=2, num_freqs=5)
